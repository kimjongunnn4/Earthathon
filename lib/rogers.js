ig.module("rogers")
  ...
  .defines(function () {
    ...
    for (var l = 0; l < _LANG_LIST.length; l++) {
      ...
      animSheetList.buttonContinue = new ig.AnimationSheet(
        "media/img/button-continue.png",
        132,
        43
      );
      ...
      animSheetList.buttonContinueImg = new ig.Image(
        "media/img/button-continue.png"
      );
      ...
      animSheetList.screenStory = new ig.Image("media/img/screen-story.png");
    ...
    EntityAsteroid = ig.Entity.extend({
      objects: [
        "media/img/glass-bottle.png",
        "media/img/bottle.png",
        "media/img/trashbag.png",
      ],
      damage: [10, 20, 30], // Adjust damage values accordingly
      num: 0,
      speed: 0,
      maxVel: { x: 0, y: 0 },
      ...
      init: function (x, y, settings) {
        this.num = Math.floor(Math.random() * this.objects.length); // Randomly select an object
        this.animSheet = new ig.AnimationSheet(this.objects[this.num], 32, 32); // Assuming a size of 32x32 for the images

        // Use a static animation if needed; can be modified based on how you want it to behave
        this.addAnim("idle", 1, [0]); // Assuming you only need a static image
    ...
    EntityButtonBlackmoon = EntityButton.extend({
      size: { x: 35, y: 40 },
      animSheet: new ig.AnimationSheet("media/img/button-ywt.png", 35, 40),
      // update: function () {
      //   if (ig.input.pressed("click") && this.inFocus()) {
      //     if (ig.Sound.enabled) this.sound.play();
    ...
    EntityButtonStart = EntityButton.extend({
      size: { x: 181, y: 52 },
      animSheet: new ig.AnimationSheet("media/img/button-start.png", 181, 52),
      update: function () {
        if (ig.input.pressed("click") && this.inFocus()) {
          if (ig.Sound.enabled) {
            this.sound.play();
    ...
    EntityButtonContinue = EntityButton.extend({
      size: { x: 132, y: 43 },
      animSheet: animSheetList.buttonContinue,
      update: function () {
        if (ig.input.pressed("click") && this.inFocus()) {
          if (ig.Sound.enabled) this.sound.play();
        }
    ...
    EntityPlayer = ig.Entity.extend({
      health: 100,
      maxHealth: 100,
      size: { x: 84, y: 84 }, // Set size according to the image dimensions
      offset: { x: 7, y: 7 },
      type: ig.Entity.TYPE.NONE,
      checkAgainst: ig.Entity.TYPE.B,
      image: new ig.Image("media/img/player-earth.png"), // Load the simple Earth image
      maxVel: { x: 0, y: 110 },
      friction: { x: 0, y: 225 },
      speed: 200,
      ascend: 140,
      ...
      draw: function () {
        this.parent();
        this.image.draw(this.pos.x, this.pos.y); // Draw the Earth image at the player's position
      },
    ...
    StartScreen = ig.Game.extend({
      ...
      background: new ig.Image("media/img/deepspace.png"),
      stars: new ig.Image("media/img/stars.png"),
      ship: new ig.Image("media/img/cover-earth.png"),
      logo: new ig.Image("media/img/cover-earth-title.png"),
      title: new ig.Image("media/img/cover-earth-title.png"),
      ywt: new ig.Image("media/img/button-ywt.png"),
      ...
      draw: function () {
        ...
        this.logo.draw((ig.system.width - this.logo.width) / 2, -130);
        // this.title.draw((ig.system.width - this.title.width + 280) / 2, 0);
    ...
    StoryScreen = ig.Game.extend({
      story: animSheetList.screenStory,
      buttonContinue: animSheetList.buttonContinueImg,
      init: function () {
        ig.input.bind(ig.KEY.SPACE, "start");
        ig.input.bind(ig.KEY.ENTER, "start");
